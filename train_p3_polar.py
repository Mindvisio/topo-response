...
dm=AtomsDataModule(DB, batch_size=128, split_file='cache/split_%s.npz'%a.split, load_properties=['polarizability'], transforms=tfs, num_workers=6, pin_memory=True)
...
print('P3-POLAR RUN tag=%s split=%s cond=%s RESUME=%s'%(tag,a.split,a.cond,resume), flush=True)
trainer.fit(task, datamodule=dm, ckpt_path=resume)
print('P3-POLAR DONE tag=%s'%tag, flush=True)
