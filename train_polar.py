...
model=NeuralNetworkPotential(representation=painn, input_modules=[PairwiseDistances()], output_modules=[polar])
...
os.makedirs('ckpt_polar', exist_ok=True)
ck=ModelCheckpoint(dirpath='ckpt_polar', filename='best-{epoch:02d}-{val_loss:.4f}', save_top_k=1, monitor='val_loss', mode='min', save_last=True, every_n_epochs=1)
trainer=pl.Trainer(accelerator='gpu', devices=1, max_epochs=60, callbacks=[ck, LearningRateMonitor()], default_root_dir='runs_polar', gradient_clip_val=10.0, log_every_n_steps=100)
resume='ckpt_polar/last.ckpt' if os.path.exists('ckpt_polar/last.ckpt') else None
print('RESUME from', resume, flush=True)
trainer.fit(task, datamodule=dm, ckpt_path=resume)
print('POLAR TRAINING DONE', flush=True)
